...
def main():
    print("=== Generating Colorful Conceptual Grid Interpolation Schematic ===")
    L = 6  # Small grid size for relaxed grid
    a_com = 0.20  # Standardized spacing
    L_bg = 128  # High resolution for identical, perfectly sharp backgrounds
    
    # 1. Initialize SkX Phase (Analytical Ansatz and high-res background)
    print("[1/2] Generating idealized SkX ansatz and high-res background...")
    _, ax_skx, ay_skx = init_SkX(L)
    spins_skx_bg, _, _ = init_SkX(L_bg)
    P_x_skx, P_y_skx = L * ax_skx, L * ay_skx
    Lx_skx_new = max(1, int(round(P_x_skx / a_com)))
    Ly_skx_new = max(1, int(round(P_y_skx / a_com)))
    
    # 2. Initialize SC Phase (Analytical Ansatz and high-res background)
    print("[2/2] Generating idealized SC ansatz and high-res background...")
    _, ax_sc, ay_sc = init_SC(L)
    spins_sc_bg, _, _ = init_SC(L_bg)
    ...
    def plot_grid(ax_plot, grid_Lx, grid_Ly, grid_ax, grid_ay, phys_Px, phys_Py, spins_bg, dot_size=45):
        # Grid coordinates
        x = np.arange(grid_Lx) * grid_ax
        y = np.arange(grid_Ly) * grid_ay
        X, Y = np.meshgrid(x, y, indexing='ij')
        
        # Plot identical high-resolution background heatmap of Sz over original physical box
        ax_plot.imshow(
            spins_bg[:, :, 2].T, 
            cmap='bwr', 
            vmin=-1, 
            vmax=1, 
            origin='lower', 
            extent=[0, phys_Px, 0, phys_Py], 
            alpha=0.28,
            interpolation='bicubic',
            zorder=1
        )
        
        # Plot grid points (plain black dots without outlines)
        ax_plot.scatter(
            X.flatten(), 
            Y.flatten(), 
            color='black', 
    ...
    # Row 0: SkX
    plot_grid(axs[0, 0], L, L, ax_skx, ay_skx, P_x_skx, P_y_skx, spins_skx_bg, dot_size=32)
    plot_arrow(axs[0, 1])
    plot_grid(axs[0, 2], Lx_skx_new, Ly_skx_new, a_com, a_com, P_x_skx, P_y_skx, spins_skx_bg, dot_size=8)
    ...
    # Row 1: SC
    plot_grid(axs[1, 0], L, L, ax_sc, ay_sc, P_x_sc, P_y_sc, spins_sc_bg, dot_size=32)
    plot_arrow(axs[1, 1])
    plot_grid(axs[1, 2], Lx_sc_new, Ly_sc_new, a_com, a_com, P_x_sc, P_y_sc, spins_sc_bg, dot_size=8)
    ...
    path_colorful = os.path.join(out_dir, "grid_interpolation_colorful.png")
    plt.savefig(path_colorful, dpi=300, bbox_inches='tight')
    
    print(f"Saved colorful schematic to '{path_colorful}'")
    plt.close()
