...
class FintempArgs:
    ...
    def __init__(self, L=32, L_super=None, H=0.0, A=0.0, T=0.1, dt=0.005, steps=1000, block=100, seed=42, no_plot=True, live_mode="quiver", dynamic_scaling=False, plot_fft=False, save_all=False, iso_scale=False, standard_a=None, grid_round=None):
        self.L = L
...
def _evaluate_fintemp_point(task):
    ...
    """
    i, j, a, h, T_sel, L, L_super, steps, block, dynamic_scaling, iso_scale, standard_a, grid_round = task
    phase_map = {"SkX": 0, "SC": 1, "SP": 2, "FM": 3}
    
    sim_args = FintempArgs(L=L, L_super=L_super, T=T_sel, steps=steps, block=block, no_plot=True, dynamic_scaling=dynamic_scaling, iso_scale=iso_scale, standard_a=standard_a, grid_round=grid_round)
    sim_args.H = h
    sim_args.A = a
...
def generate_fintemp_phase_diagram(T_sel=0.1, n_H=26, n_A=33, L=32, L_super=None, steps=1000, block=100, workers=None, dynamic_scaling=False, iso_scale=False, standard_a=None, grid_round=None):
    """
    ...
    tasks = [(i, j, a, h, T_sel, L, L_super, steps, block, dynamic_scaling, iso_scale, standard_a, grid_round) for i, a in enumerate(A_vals) for j, h in enumerate(H_vals)]
        ...
        with ProcessPoolExecutor(max_workers=workers, mp_context=mp_context) as executor:
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    i, j, *rest = future_to_task[future]
                    phase_grid[i, j] = -1
    ...
    # Save Paths
    out_dir = "output/phase_diagrams/fintemp"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--grid-round", type=int, default=None, help="Round grid sizes (Lx_new, Ly_new) to the nearest multiple of this value (e.g. 4 or 8) to prevent excessive JAX compilations")
    
    args = parser.parse_args()
    generate_fintemp_phase_diagram(T_sel=args.T, n_H=args.nH, n_A=args.nA, L=args.L, L_super=args.L_super, steps=args.steps, block=args.block, workers=args.workers, dynamic_scaling=args.dynamic_scaling, iso_scale=args.iso_scale, standard_a=args.standard_a, grid_round=args.grid_round)
